...
parser = argparse.ArgumentParser(
       prog='demo.py',
       description='''This script takes a string (Welsh text) and outputs for 
                      each token the POS and semantic tags in the form (token,POS,SEM)''')

#string argument
parser.add_argument('-s', '--text_string', help = 'Welsh text string.')
parser.add_argument('-f', '--text_file', help = 'Welsh text file.')
...
if __name__ == "__main__":
    tagr = DemoTagger()
    if args.text_file:
        with open(args.text_file, 'r', encoding='utf8') as infile:
            with open(args.text_file[:-3]+'tagged', 'w', encoding='utf8') as outfile:
                for line in infile:
                    result = tagr.tag(line)
                    outfile.write(" ".join(f"{wd}/{pt}/{st}" for wd,pt,st in result)+'\n')                 
        print(f"Tagging successful: Result written to: {args.text_file[:-3]+'tagged'}")
    elif args.text_string:
        result = tagr.tag(args.text_string)
        print(result)
